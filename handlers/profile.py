import logging
import json  # Используем json для безопасного преобразования
...
class Profile(StatesGroup):
    ...
    change_name = State()  # Добавлено состояние для изменения имени
...
async def update_phone(message: types.Message, state: FSMContext):
    if not message.text.isdigit():
        ...
        return
    logging.info("Пользователь изменил номер телефона: %s", message.text)
...
async def add_additional_phone(message: types.Message, state: FSMContext):
    ...
    if user_info:
        ...
        logging.info("Пользователь добавил дополнительный номер телефона: %s", message.text)
...
async def update_address(message: types.Message, state: FSMContext):
    logging.info("Пользователь изменил адрес доставки: %s", message.text)
...
async def update_name(message: types.Message, state: FSMContext):
    logging.info("Пользователь изменил имя: %s", message.text)
